...
storage_binaries = [
    'go/eggsblockservice/eggsblockservice',
    'scripts/startblockservice.py',
]
...
for host in metadata_hosts:
    processes.append((
        host,
        subprocess.Popen(
            ['rsync', '-p', '--mkpath', '--quiet'] + [f'{this_dir}/{binary}' for binary in metadata_binaries] + [f'REDACTED'],
        )
    ))
for host in storage_hosts:
    processes.append((
        host,
        subprocess.Popen(
            ['rsync', '-p', '--mkpath', '--quiet'] + [f'{this_dir}/{binary}' for binary in storage_binaries] + [f'REDACTED'],
        ),
    ))
