...
ffibuilder.set_source("_crc32c",
    ...
    uint32_t crc32_fusion(uint32_t crc, const char* buf, size_t length) {
        if (length >= 31) {
...
def compile(verbose=False):
    ...
    # Compile in tmp dir, then move atomically, to avoid multiple scripts
    # stepping on each others' toes.
    with tempfile.TemporaryDirectory(dir=outdir, prefix='_crc32c.') as tmpdir:
        ffibuilder.compile(tmpdir=tmpdir, verbose=verbose)
        for f in os.listdir(tmpdir):
            assert os.path.isfile(os.path.join(tmpdir, f))
            os.rename(os.path.join(tmpdir, f), os.path.join(outdir, f))
