...
func copyBlock(
	...
	defer mbs.ReleaseBlockServiceConnection(log, srcConn)
	dstConn, err := mbs.GetBlockServiceConnection(log, dstBlock.BlockServiceIp1, dstBlock.BlockServicePort1, dstBlock.BlockServiceIp2, dstBlock.BlockServicePort2)
	if err != nil {
		return 0, err
	}
	proof, err = mbs.CopyBlock(log, srcConn, blockService, block.BlockId, block.Crc32, blockSize, dstConn, dstBlock)
	defer mbs.ReleaseBlockServiceConnection(log, dstConn)
	if err != nil {
		...
		for _, span := range fileSpansResp.Spans {
			...
			for _, block := range span.BodyBlocks {
				...
				if blockService.Flags == 0 {
					...
					newBlock, err := copyBlock(log, client, mbs, scratchFile, fileSpansResp.BlockServices, span.BlockSize, span.StorageClass, &block)
					if err != nil {
						return err
					}
...
func MigrateBlocksInFile(
	...
	return migrateBlocksInFileInternal(log, client, mbs, stats, blockServiceId, &scratchFile, fileId)
}
...
func migrateBlocksInternal(
	...
	for {
		...
		for _, file := range filesResp.FileIds {
			...
			if err := migrateBlocksInFileInternal(log, client, mbs, stats, blockServiceId, &scratchFile, file); err != nil {
				return err
			}
		}
...
func MigrateBlocks(
	...
	if err := migrateBlocksInternal(log, client, mbs, stats, shid, blockServiceId); err != nil {
		return err
	}
