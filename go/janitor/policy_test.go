...
func TestKeepWithin(t *testing.T) {
	...
	assert.Equal(t, 3, (&SnapshotPolicy{DeleteAfterTime: time.Hour}).edgesToRemove(date(8), edges))
	...
	assert.Equal(t, 0, (&SnapshotPolicy{DeleteAfterTime: 48 * time.Hour * 10}).edgesToRemove(date(8), make([]msgs.EdgeWithOwnership, 0)))
}
