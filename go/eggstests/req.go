...
func createFile(
	...
	if size > 0 {
		...
		for offset := uint64(0); offset < size; offset += uint64(spanSize) {
			...
			rand.Read(*spanBuf)
			err := client.CreateSpan(log, []msgs.BlacklistEntry{}, &spanPolicy, &blockPolicies, &stripePolicy, constructResp.Id, msgs.NULL_INODE_ID, constructResp.Cookie, offset, uint32(thisSpanSize), spanBuf)
			bufPool.Put(spanBuf)
			if err != nil {
				panic(err)
			}
