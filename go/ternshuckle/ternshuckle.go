...
func (s *state) selectBlockServices(locationId *msgs.Location, id *msgs.BlockServiceId, flagsFilter msgs.BlockServiceFlags, flagsChangedSince msgs.TernTime, flagsChangedBefore msgs.TernTime, filterWithSpaceAvailable bool, hostFilter string) (map[msgs.BlockServiceId]BlockServiceInfoWithLocation, error) {
	s.semaphore.Acquire(context.Background(), 1)
	defer s.semaphore.Release(1)
	n := sql.Named
...
func (s *state) selectNonReadableFailureDomains(locationId msgs.Location) ([]string, error) {
	...
	for _, v := range bs {
		...
		switch {
		case v.Flags&msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED == msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED:
			flags = msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED
		case v.Flags&msgs.TERNFS_BLOCK_SERVICE_NO_READ == msgs.TERNFS_BLOCK_SERVICE_NO_READ:
			flags = msgs.TERNFS_BLOCK_SERVICE_NO_READ
		case v.Flags&msgs.TERNFS_BLOCK_SERVICE_STALE == msgs.TERNFS_BLOCK_SERVICE_STALE:
			flags = msgs.TERNFS_BLOCK_SERVICE_STALE
		}
		if flags != 0 {
...
func handleChangedBlockServicesAtLocation(ll *lib.Logger, s *state, req *msgs.ChangedBlockServicesAtLocationReq) (*msgs.ChangedBlockServicesAtLocationResp, error) {
	...
	resp.LastChange = msgs.MakeTernTime(time.Now().Add(-10 * time.Minute))
	i := 0
	for _, bs := range blockServices {
...
func handleRegisterBlockServices(ll *lib.Logger, s *state, req *msgs.RegisterBlockServicesReq) (*msgs.RegisterBlockServicesResp, error) {
	...
	for i := range req.BlockServices {
		...
		// DECOMMISSIONED|STALE are always managed externally, not by ternblocks.
		if msgs.BlockServiceFlags(bs.FlagsMask).HasAny(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED | msgs.TERNFS_BLOCK_SERVICE_STALE) {
		...
		res, err := tx.Exec(`
			...
			uint8(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED),
			bs.FlagsMask|uint8(msgs.TERNFS_BLOCK_SERVICE_STALE), // remove staleness -- we've just updated the BS
			bs.FlagsMask,
			uint8(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED),
			bs.FlagsMask|uint8(msgs.TERNFS_BLOCK_SERVICE_STALE),
			bs.FlagsMask,
			now,
		)
		if err != nil {
...
func handleSetBlockserviceDecommissioned(ll *lib.Logger, s *state, req *msgs.DecommissionBlockServiceReq) (*msgs.DecommissionBlockServiceResp, error) {
	...
	if bsInfo.Flags.HasAny(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED) {
...
func handleSetBlockServiceFlags(ll *lib.Logger, s *state, req *msgs.SetBlockServiceFlagsReq) (*msgs.SetBlockServiceFlagsResp, error) {
	...
	if req.FlagsMask&uint8(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED) != 0 {
		req.Flags = msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED
		req.FlagsMask = uint8(msgs.TERNFS_BLOCK_SERVICE_MASK_ALL)
	}
...
func handleInfoReq(log *lib.Logger, s *state, req *msgs.InfoReq) (*msgs.InfoResp, error) {
	...
	err := func() error {
		...
		rows, err := s.db.Query(
			...
			FROM block_services
			`,
			sql.Named("decommissioned", msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED),
		)
		if err != nil {
...
func handleEraseDecomissionedBlockReq(log *lib.Logger, s *state, req *msgs.EraseDecommissionedBlockReq) (*msgs.EraseDecommissionedBlockResp, error) {
	...
	if !ok || blockService.Flags&msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED == 0 {
...
func serviceMonitor(ll *lib.Logger, st *state, staleDelta time.Duration) error {
	...
	for {
		...
		formatLastSeen := func(t msgs.TernTime) string {
		...
		func() {
			...
			_, err := st.db.Exec(
				...
				n("flag", msgs.TERNFS_BLOCK_SERVICE_STALE),
				n("flags_last_changed", uint64(now)),
				n("thresh", thresh),
				n("decom_flag", msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED),
			)
			if err != nil {
		...
		func() {
			...
			rows, err := st.db.Query(
				...
				n("d_flag", msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED),
				// if it was manually marked as non read/non write it's most likely being worked on. no point to alert
				n("nr_nw_flags", msgs.TERNFS_BLOCK_SERVICE_NO_READ|msgs.TERNFS_BLOCK_SERVICE_NO_WRITE),
			)
			if err != nil {
...
func blockServiceAlerts(log *lib.Logger, s *state) {
	...
	for {
		...
		for _, bs := range blockServices {
			...
			if bs.Flags.HasAny(msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED) {
				continue
			}
		...
		for _, bs := range blockServices {
			...
			if bs.HasFiles && bs.FlagsLastChanged < msgs.MakeTernTime(time.Now().Add(-36*time.Hour)) {
...
func initBlockServicesTable(db *sql.DB) error {
	...
	_, err = db.Exec("UPDATE block_services SET flags = :decom_flag WHERE (flags & :decom_flag) <> 0", n("decom_flag", msgs.TERNFS_BLOCK_SERVICE_DECOMMISSIONED))
	return err
}
